...
def load_quantized_model_gguf_ggml(model_basename, device_type, logging):
    """
    ...
    """
    model_file_path = os.path.join(MODELS_PATH, model_basename)
    try:
...
def load_quantized_model_gptq(model_basename, logging):
    """
    ...
    if sys.platform == "darwin":
        logging.INFO(
            "GPTQ models will NOT work on Mac devices. Please choose a different model."
        )
        return None, None
    ...
    model_file_path = os.path.join(MODELS_PATH, model_basename)
    tokenizer = AutoTokenizer.from_pretrained(model_file_path, use_fast=True)
...
def load_full_model(model_basename, device_type, logging):
    """
...
def load_quantized_model_awq(model_id, logging):
    ...
    if sys.platform == "darwin":
        logging.INFO(
            "AWQ models will NOT work on Mac devices. Please choose a different model."
        )
        return None, None
