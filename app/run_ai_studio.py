...
def load_model(device_type, model_basename, LOGGING=logging):
    """
    ...
    """
    logging.info(f"Loading Model: {model_basename}, on: {device_type}")
    logging.info("This action can take a few minutes!")

    if "gguf" in model_basename.lower():
        llm = load_quantized_model_gguf_ggml(model_basename, device_type, LOGGING)
        return llm
    ...
    #     model, tokenizer = load_quantized_model_awq(model_id, LOGGING)
        ...
        model, tokenizer = load_quantized_model_gptq(model_basename, LOGGING)
    else:
        model, tokenizer = load_full_model(model_basename, device_type, LOGGING)
...
def retrieval_qa_pipline(device_type, use_history, promptTemplate_type="llama"):
    ...
    llm = load_model(device_type, model_basename=MODEL_BASENAME, LOGGING=logging)
...
def main(device_type, show_sources, use_history, model_type, save_qa):
    ...
    while True:
        ...
        if save_qa:
            utils.log_to_csv(MODEL_BASENAME, time_taken, query, answer)
...
if __name__ == "__main__":
    ...
    models = {
        "a": "bert-base-cased-squad2",
        "b": "zephyr-7b-beta",
        "c": "zephyr-7B-beta-GPTQ",
        "d": "CapybaraHermes-2.5-Mistral-7B-GPTQ",
        "e": "Mistral-7B-Instruct-v0.2-GGUF",
        "f": "zephyr-7B-beta-AWQ",
    }
